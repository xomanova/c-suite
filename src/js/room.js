...
(function() {
  var KEEP_FREQUENT_NUM, KEEP_RECENT_NUM, ScalarKalmanFilter, adjectives, adverbs, dot_product, extend_phase, get_player_interactions, join_room, games, normalize, nouns, safe_localStorage_access, save_player_interactions, silly_sentence, to_degrees, to_radians, update_room, update_time_difference_filter, vector_length, verbs,
    slice = [].slice,
  ...
  games.has_touch = 'ontouchstart' in window || 'msMaxTouchPoints' in window.navigator;

  games.key_value = function(key, value) {
    var object;
    object = {};
    object[key] = value;
    return object;
  ...
  games.to_server_timestamp = function(client_timestamp) {
    return client_timestamp + games.time_difference;
  };

  games.to_client_timestamp = function(server_timestamp) {
    return server_timestamp - games.time_difference;
  };

  games.player_by_id = function(players, player_id) {
    console.log("logging from games.render room.js:30, players typeof: " + typeof players );
    console.log("logging from games.render room.js:31, players: " + JSON.stringify(players) );
    var i, len, player;
  ...
  games.comma_list = function(list, comma, final_separator) {
    var first, i, last;
    if (comma == null) {
      comma = ', ';
    }
  ...
  games.shame_players = function(object, players, shameful) {
    return players.filter(function(player) {
      return object.hasOwnProperty(player.id) && shameful(object[player.id]);
    });
  };

  games.generate_shame_text = function(shame_players, no_shame_text, shame_threshold) {
    ...
    if (shame_players.length === 0) {
    ...
    } else if (games.player_by_id(shame_players, games.player.id) != null) {
      ...
      return 'Waiting for ' + games.comma_list(shame_players.map(function(player) {
        return player.name;
      }));
    } else {
  ...
  games.player_name_or_you = function(player, arg) {
  ...
  games.render_list = function($list, $template, values, render) {
  ...
  games.render_players = function(state, players, $players, class_predicates) {
    return games.render_list($players, $('<li>'), players, (function(_this) {
  ...
  games.render_theme = function($content, theme) {
  ...
  games.register_player_interactions = function(timestamp, players) {
  ...
  games.change_name_storage = function(player_name) {
    var ref;
  ...
  games.change = function(state) {
    return games.socket.send(JSON.stringify({
      action: 'change',
  ...
  games.change_name = function(player_name) {
    if (!games.change_name_storage(player_name)) {
      return false;
    }
  ...
  games.boot = function(player_ids) {
    return games.socket.send(JSON.stringify({
      action: 'boot',
      room_id: games.room_id,
  ...
  games.measure_time_difference = function() {
    return games.socket.send(JSON.stringify({
  ...
  games.refresh_if = function(needs_refresh) {
    return safe_localStorage_access(function() {
      if (needs_refresh) {
  ...
  games.render = function(state, players) {
    var $content, $section, $spectator_base, $utility_menu, phase, player, ref, spectator;
    state = state;
    //players = JSON.parse(players);
    console.log("logging from games.render room.js:364, state typeof: " + typeof state );
    console.log("logging from games.render room.js:365, state: " + state );
    ...
    console.log("logging from games.render room.js:369, games: " + JSON.stringify(games) );
    ...
    games.render_players(state, games.room.spectators, $utility_menu.find('.spectators'), {
      'current-player': function(state, spectators, spectator) {
        return spectator.id === games.player.id;
      },
      'boot-enabled': function() {
        return players[0].id === games.player.id;
      }
    });
    $spectator_base = $('#spectator-base');
    $spectator_base.toggleClass('can-join', games.room.can_join);
    if (typeof games.prerender === "function") {
  ...
  games.refresh = function() {
    ...
    $section = $('#' + state.phase);
    phase = games.phases[state.phase];
    return phase != null ? typeof phase.refresh === "function" ? phase.refresh(state, games.players, $section) : void 0 : void 0;
  };
  ...
  games.utility_menu_player_class_predicates = {
    'current-player': function(state, players, player) {
      return player.id === games.player.id;
    },
    'boot-enabled': function(state, players, player) {
      return players[0].id === games.player.id && players[0].id !== player.id && player.can_leave;
    }
  };

  games.add_phases = function(phases) {
    var name, phase, results;
    results = [];
    for (name in phases) {
      phase = phases[name];
      results.push(games.phases[name] = phase);
    }
    return results;
  };

  games.add_utility_menu_player_class_predicates = function(class_predicates) {
    var klass, predicate, results;
    results = [];
    for (klass in class_predicates) {
      predicate = class_predicates[klass];
      results.push(games.utility_menu_player_class_predicates[klass] = predicate);
    }
    return results;
  };
  ...
  games.lib.huddle = function(arg, additional) {
    ...
    console.log("logging from games.lib.huddle room.js:513, arg: " + JSON.stringify(arg) );
    console.log("logging from games.lib.huddle room.js:514, additional: " + JSON.stringify(additional) );
    ...
    return extend_phase({
      ...
      method: function(state, players, $section) {
        ...
        is_second = ((ref11 = players[1]) != null ? ref11.id : void 0) === games.player.id;
  ...
  games.lib.wait = function(arg, additional) {
  ...
  games.lib.wait_all = function(arg, additional) {
    ...
    ref = arg != null ? arg : {}, selector = (ref1 = ref.selector) != null ? ref1 : '.btn', text = (ref2 = ref.text) != null ? ref2 : 'OK', is_disabled = (ref3 = ref.is_disabled) != null ? ref3 : function(state, players) {
      ...
      return games.shame_players(state.ready, players, function(ready) {
        return !ready;
      });
    };
    if (additional == null) {
    ...
    return extend_phase({
      ...
      method: function(state, players, $section) {
        ...
        $button = $section.find(selector);
        ready = (ref7 = state.ready[games.player.id]) != null ? ref7 : false;
        disabled = is_disabled(state, players);
        $button.toggleClass('disabled', ready || disabled);
        return $button.text(disabled ? (ref8 = get_disabled_text(state, players)) != null ? ref8 : text : !ready ? text : games.generate_shame_text(get_shame_players(state, players), get_no_shame_text(state, players)));
      }
    }, additional);
  };

  games.lib.pass = function(arg, additional) {
    ...
    ref = arg != null ? arg : {}, selector = (ref1 = ref.selector) != null ? ref1 : '.btn', text = (ref2 = ref.text) != null ? ref2 : 'Pass', get_shame_players = (ref3 = ref.get_shame_players) != null ? ref3 : function(state, players) {
      return games.shame_players(state.pass, players, function(val) {
        return !val;
      });
    };
    if (additional == null) {
    ...
    return extend_phase({
      action: function() {
        ...
        return games.change({
          pass: games.key_value(games.player.id, $(this).is('.btn-default'))
        });
      },
      attach: function($section) {
        return $section.find(selector).click(this.action);
      ...
      method: function(state, players, $section) {
        ...
        return $button.text(!pass ? text : games.generate_shame_text(shame_players));
      }
    }, additional);
  };

  games.lib.select_player = function(arg, additional) {
    ...
    ref = arg != null ? arg : {}, valid_target = (ref1 = ref.valid_target) != null ? ref1 : '.enabled a:not(.disabled)', target_property = (ref2 = ref.target_property) != null ? ref2 : 'target', list_selector = (ref3 = ref.list_selector) != null ? ref3 : '.list-group', ok_selector = (ref4 = ref.ok_selector) != null ? ref4 : '.btn', $player_template = (ref5 = ref.$player_template) != null ? ref5 : $('<a class="list-group-item">\n  <span class="player-name"></span>\n  <span class="label label-default"></span>\n</a>'), get_id = (ref6 = ref.get_id) != null ? ref6 : function(player, index) {
      ...
      return games.key_value(target_property, $player.is('.active') ? null : id);
    ...
    return extend_phase({
      action: function() {
        var $player;
        $player = $(this);
        return games.change(change(target_property, $player, $player.data('id')));
      },
      attach: function($section) {
      ...
      method: function(state, players, $section) {
        ...
        return games.render_list($players, $player_template, filtered_players, function($player, player, index) {
          var player_id;
  ...
  games.lib.multiplayer_vote = function(arg, additional) {
    ...
    return extend_phase({
      action: function() {
        return games.change(games.key_value(vote_property, games.key_value(games.player.id, $(this).is(yes_selector))));
      },
      attach: function($section) {
      ...
      render: function(state, players, $section) {
        ...
        if (shame_selector != null) {
          has_voted = state[vote_property][games.player.id] != null;
          shame_players = games.shame_players(state[vote_property], players, function(val) {
            return val == null;
          });
          $shame_text = $section.find(shame_selector);
          $shame_text.toggle(has_voted);
          return $shame_text.text(games.generate_shame_text(shame_players));
        }
      }
    }, additional);
  };

  games.lib.choose_word_packs = function(arg, additional) {
    ...
    return extend_phase(games.lib.wait({
      ...
      attach: function($section) {
        return $section.find(pack_selector).click(function(event) {
          ...
          return games.change({
            chosen_packs: games.key_value(pack_name, !selected)
          });
        });
      }
    }), additional);
  ...
  games.lib.two_teams = function(arg, additional) {
    ...
    return extend_phase(games.lib.wait({
      ...
      attach: function($section) {
        $section.find(blue_selector).on('click', function() {
          return games.change({
            team: games.key_value(games.player.id, 'blue')
          });
        });
        return $section.find(red_selector).on('click', function() {
          return games.change({
            team: games.key_value(games.player.id, 'red')
          });
        });
      },
      render: function(state, players, $section) {
        ...
        render_team = function(player_ids, $players) {
          return games.render_players(state, players.filter(function(player) {
            var ref7;
  ...
  games.lib.terminal = function(arg, additional) {
  ...
  games.lib.hide_flat = function(arg, additional) {
  ...
  games.lib.rotation = function(arg, additional) {
  ...
  games.lib.rotation_screens = function(arg, additional) {
  ...
  games.lib.clock = function(arg, additional) {
    ...
    return extend_phase({
      refresh: function(state, players) {
        ...
        return setTimeout(games.refresh, 1000 - elapsed_millis % 1000);
      }
    }, additional);
  };
  ...
  update_time_difference_filter = function(client_timestamp, server_timestamp) {
    ...
    return games.time_difference = games.time_difference_filter.time_difference;
  };

  update_room = function(room) {
    //var clock_difference, ref, ref1, ref2, ref3, ref4, ref5, same_room, time_difference;
    //same_room = ((ref = games.room) != null ? ref.created : void 0) === room.created;
    //clock_difference = ((ref1 = (ref2 = games.room) != null ? (ref3 = ref2.clock) != null ? ref3.server : void 0 : void 0) != null ? ref1 : 0) - room.clock.server;
    //time_difference = ((ref4 = (ref5 = games.room) != null ? ref5.last_modified : void 0) != null ? ref4 : 0) - room.last_modified;
    ...
    //}
    games.room = room;
    return games.render(games.room.state, games.room.players);
  };

  join_room = function() {
    var host, room_id, socket;
    room_id = games.room_id = localStorage.room != null ? localStorage.room : $('#room-id').val();
    host = window.location.host;

    socket = games.socket = new WebSocket('wss://' + window.games_host + '/socket/');
    socket.onopen = function() {
      $('#connecting').hide();
      if (games.time_difference_filter == null) {
...
//    }
    ...
    socket.onmessage = function(arg) {
      console.log('Inside onmessage two, games:' + JSON.stringify(games));
    ...
    return socket.onerror = function(message) {
      console.log('Inside onerror, games:' + JSON.stringify(games))
      $('#error-message').text(message);
      return console.error(message);
    };

    //socket.addEventListener("message", function(event) {
    //  console.log('Inside addedEventListener, games:' + JSON.stringify(games))
  ...
  $(function() {
    ...
    games.player.id = (ref1 = safe_localStorage_access(function() {
      return localStorage.player_id;
    })) != null ? ref1 : new_player_uuid;
    games.player.name = safe_localStorage_access(function() {
      return localStorage.player_name;
    });
    safe_localStorage_access(function() {
      if (localStorage.player_id == null) {
        return localStorage.player_id = games.player.id;
      }
    });
    if (games.player.name != null) {
      join_room();
    ...
    $utility_menu.find('.spectate').click(function(event) {
      close_utility_menu();
      return games.boot([games.player.id]);
    });
